# Progress Tracker

> This file is the implementation state. AI coding agents must read it before changing the project.

## Status

- ⬜ Not started
- 🟡 In progress
- 🟢 Verified
- 🔴 Blocked
- ⚠️ Needs review

## Phase 0 — Understanding
- 🟢 PROJECT_SPEC reviewed
- 🟢 Architecture reviewed
- 🟢 Domain model reviewed
- 🟢 Current task identified

## Phase 1 — Foundation
- 🟢 Repository
- 🟢 Workspace
- 🟢 TypeScript
- 🟢 Lint
- 🟢 Formatting
- 🟢 Environment
- 🟢 CI

## Phase 2 — Domain
- 🟢 Domain model finalized
- 🟢 State machines finalized
- 🟢 Relationships verified
- 🟢 Idempotency rules verified

## Phase 3 — Database
- 🟢 Prisma schema
- 🟢 PostgreSQL
- 🟢 Migration
- 🟢 Seed
- 🟢 Database tests

## Phase 4 — Security
- 🟢 Authentication
- 🟢 RBAC
- 🟢 Tenant isolation
- 🟢 Protected APIs

## Phase 5 — Payments
- 🟢 Payment
- 🟢 PaymentAttempt
- 🟢 PaymentFailure
- 🟢 Idempotency
- 🟢 UNKNOWN handling
- 🟢 State machine

## Phase 6 — Recovery
- 🟢 RevenueRisk
- 🟢 RecoveryCase
- 🟢 Recovery lifecycle
- ⬜ ML prediction

## Phase 7 — AI
- ⬜ Agent interface
- ⬜ Structured output
- ⬜ Validation
- ⬜ Version tracking

## Phase 8 — Policy
- ⬜ Policy
- ⬜ Policy evaluation
- ⬜ PolicyDecision
- ⬜ Stopping rules

## Phase 9 — Workflow
- ⬜ RecoveryAction
- ⬜ Durable execution
- ⬜ Retry
- ⬜ Timeout
- ⬜ Idempotency
- ⬜ Terminal outcomes

## Phase 10 — Observability
- 🟢 AuditEvent
- 🟢 Correlation IDs
- ⬜ Trace IDs
- ⬜ Notifications
- ⬜ Metrics

## Phase 11 — UI
- ⬜ Dashboard
- ⬜ Cases
- ⬜ Case detail
- ⬜ Payment detail
- ⬜ Timeline

## Phase 12 — Evaluation
- ⬜ Synthetic data
- ⬜ Money recovered
- ⬜ Precision/recall
- ⬜ False-positive cost
- ⬜ Unresolved exceptions

## Phase 13 — Final
- ⬜ Reliability tests
- ⬜ README
- ⬜ Demo
- ⬜ Public repository
- ⬜ Pitch video

## Current Work

**Phase:** 7 — AI Recovery Agent & Policy Guardrails  
**Task:** ML Prediction & AI Agent Recommendation Service  
**Last verified:** Payment Service Ingestion & Recovery Case Initialization

## Agent Rule

Do not mark a task `🟢 Verified` merely because code was written. Verification requires appropriate tests/checks to pass.
